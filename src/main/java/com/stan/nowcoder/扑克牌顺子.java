...
public class 扑克牌顺子 {
    ...
    public boolean isContinuous(int [] numbers) {
        ...
        int tolerance = i;  //百搭牌数量
        int offset = numbers[N - 1] - numbers[i];
        //比如5个数 第一个1 最后一个6，肯定不是顺子
        if (offset >= N) return false;

        i++;
        for (; i < N; ++i) {
            if (numbers[i] == numbers[i - 1]) return false; //对子返回
            int con = numbers[i] - numbers[i - 1] - 1;
            tolerance -= con;  //相邻的两个数之差>1，需要消耗0的数量
